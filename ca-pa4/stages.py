...
class IF(Pipe):
    ...
    def compute(self):
        ...
        if opcode not in [ILLEGAL, EBREAK, ECALL] :
            br_type = csignals[opcode][CS_BR_TYPE]
            if  br_type in [BR_NE, BR_EQ, BR_GE, BR_GEU, BR_LT, BR_LTU] :   #Backward ==> Taken, Forward ==> Not Taken
                bt = Pipe.cpu.adder_if.op(self.pc, RISCV.imm_b(self.inst))
                if (bt < self.pc) :
                    self.prediction    = bt
                else :
                    self.prediction    = self.pcplus4
            elif br_type == BR_J :                                          #Always Taken
                self.prediction = Pipe.cpu.adder_brtarget.op(self.pc, RISCV.imm_j(self.inst))
                if RISCV.rd(self.inst) == 0x1:
                    Pipe.cpu.ras.push(self.pcplus4)
            elif br_type == BR_JR:                                          #Always not Taken
                self.prediction = self.pcplus4
                if RISCV.rd(self.inst) == 0x1:
                    Pipe.cpu.ras.push(self.pcplus4)
                elif RISCV.rd(self.inst) == 0x0 and RISCV.rs1(self.inst) == 0x1 and RISCV.imm_i(self.inst) == 0:
                    stkPc, stkStatus = Pipe.cpu.ras.pop()
                    if (stkPc == 0 and stkStatus == False): 
                        self.prediction = self.pcplus4
                    else:
                        self.prediction = stkPc
            else :
                self.prediction = self.pcplus4
        else:
            self.prediction = self.pcplus4
        ...
        if Pipe.ID.c_mispredict:
            self.pc_next =  Pipe.EX.pcplus4             if Pipe.ID.c_pc_sel == PC_4     else \
                            Pipe.EX.brjump_target       if Pipe.ID.c_pc_sel == PC_BRJMP else \
                            Pipe.EX.jump_reg_target     if Pipe.ID.c_pc_sel == PC_JALR  else \
                            WORD(0)
        else : self.pc_next = self.prediction


    def update(self):
        
        if not Pipe.ID.c_IF_stall:
            IF.reg_pc          = self.pc_next

        if (Pipe.ID.c_ID_bubble and Pipe.ID.c_ID_stall):
            exit()

        if (Pipe.ID.c_ID_bubble):
            ID.reg_pc          = self.pc
            ID.reg_inst        = WORD(BUBBLE)
            ID.reg_exception   = WORD(EXC_NONE)
            ID.reg_pcplus4     = WORD(0)

        elif not Pipe.ID.c_ID_stall:
            ID.reg_pc          = self.pc
...
class ID(Pipe):
    ...
    def compute(self):
        ...
        #forward detect
        self.c_fwd_op1  =   FWD_EX      if (EX.reg_rd == self.rs1) and (self.c_rs1_oen) and (EX.reg_rd != 0) and (EX.reg_c_rf_wen)       else \
                            FWD_M1      if (M1.reg_rd == self.rs1) and (self.c_rs1_oen) and (M1.reg_rd != 0) and (Pipe.M1.c_rf_wen)      else \
                            FWD_M2      if (M2.reg_rd == self.rs1) and (self.c_rs1_oen) and (M2.reg_rd != 0) and (Pipe.M2.c_rf_wen)      else \
                            FWD_WB      if (WB.reg_rd == self.rs1) and (self.c_rs1_oen) and (WB.reg_rd != 0) and (WB.reg_c_rf_wen)       else \
                            FWD_NONE

        self.c_fwd_op2  =   FWD_EX      if (EX.reg_rd == self.rs2) and (EX.reg_rd != 0) and (EX.reg_c_rf_wen) and (self.c_op2_sel == OP2_RS2)    else \
                            FWD_M1      if (M1.reg_rd == self.rs2) and (M1.reg_rd != 0) and (Pipe.M1.c_rf_wen) and (self.c_op2_sel ==OP2_RS2)   else \
                            FWD_M2      if (M2.reg_rd == self.rs2) and (M2.reg_rd != 0) and (Pipe.M2.c_rf_wen) and (self.c_op2_sel ==OP2_RS2)   else \
                            FWD_WB      if (WB.reg_rd == self.rs2) and (WB.reg_rd != 0) and (WB.reg_c_rf_wen) and (self.c_op2_sel == OP2_RS2)    else \
                            FWD_NONE 

        self.c_fwd_rs2  =   FWD_EX      if (EX.reg_rd == self.rs2) and (self.c_rs2_oen) and (EX.reg_rd != 0) and (EX.reg_c_rf_wen)       else \
                            FWD_M1      if (M1.reg_rd == self.rs2) and (self.c_rs2_oen) and (M1.reg_rd != 0) and (Pipe.M1.c_rf_wen)      else \
                            FWD_M2      if (M2.reg_rd == self.rs2) and (self.c_rs2_oen) and (M2.reg_rd != 0) and (Pipe.M2.c_rf_wen)      else \
                            FWD_WB      if (WB.reg_rd == self.rs2) and (self.c_rs2_oen) and (WB.reg_rd != 0) and (WB.reg_c_rf_wen)       else \
                            FWD_NONE
    
        load_use_hazard =   ((EX.reg_c_dmem_en and EX.reg_c_dmem_rw == M_XRD and EX.reg_rd != 0) and \
                            ((EX.reg_rd == self.rs1 and self.c_rs1_oen) or (EX.reg_rd == self.rs2 and self.c_rs2_oen))) or\
                            ((EX.reg_rd == self.rs1 and self.c_rs1_oen) or (EX.reg_rd == self.rs2 and self.c_rs2_oen)) and\
                            ((M1.reg_c_dmem_en and M1.reg_c_dmem_rw == M_XRD and M1.reg_rd != 0))

        m1_m2_hazard    =   (self.c_dmem_en and self.EX.rd != 0) and (EX.reg_c_dmem_en and EX.reg_rd != 0)

        pred_miss       =   Pipe.EX.br_outcome != self.pc and Pipe.EX.br_outcome != WORD(0)

        self.c_mispredict   =   pred_miss
        self.c_IF_stall     =   load_use_hazard  or m1_m2_hazard 
        self.c_ID_stall     =   load_use_hazard  or m1_m2_hazard 
        self.c_ID_bubble    =   pred_miss
        self.c_EX_bubble    =   load_use_hazard or m1_m2_hazard or pred_miss
        #-------------------------------------------------------------
...
class EX(Pipe):
    ...
    def compute(self):
        ...
        if (self.inst != BUBBLE):
            self.br_outcome   = self.brjump_target      if  (self.c_br_type == BR_NE  and (not self.alu_out)) or        \
                                ...
                                self.jump_reg_target    if  (self.c_br_type == BR_JR)                           else    \
                                self.pcplus4        
        else :self.br_outcome    = WORD(0)
